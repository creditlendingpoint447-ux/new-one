...
function loadUsers() {
    ...
    const defaultUsers = [
        ...
        { username: 'Shalaine', password: 'Securesantiago', fullName: 'Shalaine Santiago', email: 'shalaine@example.com', accountType: 'Chase Freedom Unlimited®' },
...
function resetToDefaultUsers() {
    if (confirm('⚠️ Warning: This will delete all created accounts and restore default users only. Continue?')) {
        const defaultUsers = [
            { username: 'john', password: 'demo123', fullName: 'John', email: 'john@example.com', accountType: 'Chase Total Checking®' },
            { username: 'james', password: 'demo123', fullName: 'James', email: 'james@example.com', accountType: 'Chase Savings℠' },
            { username: 'Shalaine', password: 'Securesantiago', fullName: 'Shalaine Santiago', email: 'mary@example.com', accountType: 'Chase Freedom Unlimited®' },
            { username: 'marymarquez', password: '7783', fullName: 'Mary', email: 'mary@example.com', accountType: 'Chase Total Checking®' }
        ];
        localStorage.setItem('bankUsers', JSON.stringify(defaultUsers));
        users = loadUsers();
...
// ========== DEMO LOGIN ==========
function demoLogin() {
    const user = users.find(u => u.username === 'Shalaine' && u.password === 'Securesantiago');
    
    if (user) {
        currentUser = user;
        showLoading('Logging in as Shalaine Santiago...');
